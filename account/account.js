(() => {
  ...
  async function room() {
    ...
    const form=node('form',undefined,'room-form account-card'),heading=node('h2','Could this be your dog?'),label=node('label','What would you call them?'),name=node('input');name.required=true;name.maxLength=32;name.name='dog-name';name.autocomplete='off';label.append(name);const save=node('button','Save this dog and name','button primary');save.type='submit';form.append(heading,label,save,node('p','This saves your choice. It does not mint or transfer the artwork. Final adoption will open here when minting is connected.','account-note'),node('p','The painting. The idea behind it. What it means to you. After the NFT arrives in your wallet, you have seven days to pay Wubbushi what you believe the work is worth. Payment does not unlock or deliver your dog. Your valuation becomes part of the artwork. Nothing is due now.','account-note'));content.append(form);
