(() => {
  ...
  async function room() {
    ...
    const form=node('form',undefined,'room-form account-card'),heading=node('h2','Could this be your dog?'),label=node('label','What would you call them?'),name=node('input');name.required=true;name.maxLength=32;name.name='dog-name';name.autocomplete='off';label.append(name);const save=node('button','Save this dog and name','button primary');save.type='submit';form.append(heading,label,save,node('p','This saves your choice. It does not mint or transfer the artwork. Final adoption will open here when minting is connected.','account-note'),node('p','Art first. Price second. After the NFT arrives in your wallet, you have seven days to choose what to pay Wubbushi for the art. Payment does not unlock or deliver your dog. Nothing is due now.','account-note'));content.append(form);
