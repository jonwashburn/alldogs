(() => {
  ...
  async function room() {
    ...
    const form=node('form',undefined,'room-form account-card'),heading=node('h2','Could this be your dog?'),label=node('label','What would you call them?'),name=node('input');name.required=true;name.maxLength=32;name.name='dog-name';name.autocomplete='off';label.append(name);const save=node('button','Save this dog and name','button primary');save.type='submit';form.append(heading,label,save,node('p','This saves your choice. It does not mint or transfer the artwork. Final adoption will open here when minting is connected.','account-note'),node('p','Participation in ALL DOGS, a conceptual artwork by Wubbushi. You receive your dog first, then decide what the experience is worth to you. You have seven days from adoption to pay Wubbushi the amount you choose. Your valuation becomes part of the artwork. Nothing is due now.','account-note'));content.append(form);
