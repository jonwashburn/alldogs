...
(async()=>{
  ...
  const room=ui('viewing-room',true,{hasInvitation:true},{room:roomData});await settle();assert.equal(room.get('account-content').querySelectorAll('img').length,1);assert.match(room.text(),/does not mint/);assert.match(room.text(),/After adoption, you’ll be asked to pay Wubbushi the value you choose within seven days/);assert.match(room.text(),/Nothing is due now/);
  ...
  for(const count of [1,2]){
    const h=ui('viewing-room',true,{hasInvitation:true},{room:{...roomData,dogs:roomData.dogs.slice(0,count)}});await settle();assert.equal(h.get('account-content').querySelectorAll('img').length,1);assert.match(h.text(),new RegExp('1 of '+count));tests++;
  }
  for(const dogs of [[],[...roomData.dogs,roomData.dogs[0]],[roomData.dogs[0],roomData.dogs[0]]]){
  ...
  const privateGift=ui('viewing-room',true,{hasInvitation:true},{room:{...roomData,adoptionKind:'artist_gift'}});await settle();assert.match(privateGift.text(),/Artist gift\. No payment is due/);assert.doesNotMatch(privateGift.text(),/seven days|pay Wubbushi/);tests++;
