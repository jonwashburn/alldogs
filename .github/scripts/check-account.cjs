...
(async()=>{
  ...
  const personal=ui('lounge',true,{hasInvitation:true,application:null});await settle();assert.match(personal.text(),/Wubbushi has chosen three dogs/);assert.doesNotMatch(personal.text(),/Link your application|Start here/);assert.ok(personal.get('account-content').querySelectorAll('a').some(a=>a.href==='/viewing-room/'));tests++;
