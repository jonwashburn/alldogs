...
(async()=>{
  ...
  const room=ui('viewing-room',true,{hasInvitation:true},{room:roomData});await settle();assert.equal(room.get('account-content').querySelectorAll('img').length,1);assert.match(room.text(),/does not mint/);assert.match(room.text(),/After the NFT arrives in your wallet/);assert.match(room.text(),/Payment does not unlock or deliver your dog/);
