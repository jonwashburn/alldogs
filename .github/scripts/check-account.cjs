...
(async()=>{
  ...
  const room=ui('viewing-room',true,{hasInvitation:true},{room:roomData});await settle();assert.equal(room.get('account-content').querySelectorAll('img').length,1);assert.doesNotMatch(room.text(),/payment|pay Wubbushi|Nothing is due|mint|wallet/i);
  ...
  const pending=ui('viewing-room',true);await settle();assert.match(pending.text(),/Your invitation will introduce the dogs Wubbushi has chosen for you/);assert.ok(!pending.calls.some(c=>c.path==='club/room'));tests++;
