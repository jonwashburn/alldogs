...
(async()=>{
  let tests=0;
  let h=fixture();await h.ready;assert.match(h.host.text(),/The dog is yours\. The value is yours to decide/);assert.match(h.host.text(),/Your dog is home/);assert.match(h.host.text(),/pay Wubbushi, choose an amount below/);assert.match(h.host.text(),/one day from adoption/);assert.match(h.host.text(),/amount received becomes your dog’s recorded value/);assert.doesNotMatch(h.host.children.slice(2,5).map(n=>n.text()).join(' '),/NFT/i);assert.match(h.host.text(),/Please contact Wubbushi to arrange your payment/);assert.equal(h.host.all('form').length,0);tests++;
