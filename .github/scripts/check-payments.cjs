...
(async()=>{
  let tests=0;
  let h=fixture();await h.ready;assert.match(h.host.text(),/The dog is yours\. The value is yours to decide/);assert.match(h.host.text(),/Participation in ALL DOGS, a conceptual artwork by Wubbushi/);assert.match(h.host.text(),/You’ve received your dog/);assert.match(h.host.text(),/what the experience is worth to you/);assert.match(h.host.text(),/pay Wubbushi the amount you choose/);assert.match(h.host.text(),/seven days from adoption/);assert.match(h.host.text(),/Your valuation becomes part of the artwork/);assert.doesNotMatch(h.host.children.slice(2,5).map(n=>n.text()).join(' '),/NFT/i);assert.match(h.host.text(),/Card payments are being connected/);assert.equal(h.host.all('form').length,0);tests++;
