(() => {
  ...
  function label(a){if(a.status==='adopted'||a.gift?.status==='accepted')return 'Adopted';if(['rejected','withdrawn'].includes(a.status))return a.status;if(a.mail&&['unknown','failed'].includes(a.mail.status)&&invited(a))return 'Email needs attention';if(invited(a))return a.invitation?.dogName?'Dog chosen':'Invited';if(a.viewing?.status==='draft')return 'Draft';return a.vouch?.valid?'Ready to invite':'Awaiting a vouch';}
  ...
  function renderDetail(){
    ...
    const header=el('div',undefined,'detail-header'),heading=el('div');heading.append(el('p','Application · '+date(a.createdAt),'eyebrow'),el('h2','@'+(a.currentHandle||a.handle)),el('p',a.identityVerified?'X account verified':'Legacy application: X identity has not been verified.','muted'));header.append(heading,badge(label(a),a.vouch?.valid?'good':''));root.append(header);
